...
class GenericDecorator:
    ...
    def __init__(self, func_or_arg=None, *args, **kwargs):
        self._wrapped_func = None

        # Handle @decorator usage without parentheses:
        # func_or_arg is the decorated function itself
        if (
            func_or_arg is not None
            and callable(func_or_arg)
            and not args
            and not kwargs
        ):
            for k, v in self.default_kwargs.items():
                setattr(self, k, v)
            self._wrapped_func = self.modify_function(func_or_arg)
            return

        # Handle @decorator(...) usage with arguments
        all_args = (func_or_arg,) + args if func_or_arg is not None else args
    ...
    def __call__(self, func_or_self=None, *args, **kwargs):
        if self._wrapped_func is not None:
            # Already decorated (no-args usage). When accessed as a
            # descriptor on an instance, func_or_self is the instance.
            if func_or_self is not None:
                return self._wrapped_func(func_or_self, *args, **kwargs)
            return self._wrapped_func(*args, **kwargs)
        return self.modify_function(func_or_self)

    def __get__(self, obj, objtype=None):
        """Descriptor support so decorated methods work on instances."""
        if obj is None:
            return self
        if self._wrapped_func is not None:
            import functools

            bound = functools.partial(self._wrapped_func, obj)
            # Carry decoration attributes so _logic_processing can find them.
            # Check the decorator instance first (for post-decoration assignments
            # like `anchor_charts.order = 99`), then fall back to the wrapped func.
            bound._prefix = getattr(self._wrapped_func, "_prefix", "")
            bound.order = getattr(
                self, "order", getattr(self._wrapped_func, "order", 5)
            )
            return bound
        return self
